...
def make_render_pixels_uint8(env_params, static_params: StaticEnvParams):
    ...
    FIXATED_COLOUR = jnp.array([80, 80, 80], dtype=jnp.uint8)
    ...
    def colour_thruster_texture(colour):
        tex = THRUSTER_TEXTURE_16_RGBA.at[:9, :, :3].mul(colour[None, None, :] / 255.0)
        tex = jnp.round(tex).astype(jnp.uint8)
        return tex
    ...
    ROLE_COLOURS = jnp.array(
        [
            [160, 160, 160],  # None
            [0, 204, 0],  # Green:    The ball
            [0, 102, 204],  # Blue:   The goal
            [255, 102, 102],  # Red:      Death Objects
        ],
        dtype=jnp.uint8,
    )

    BACKGROUND_COLOUR = jnp.array([255, 255, 255], jnp.uint8)
    ...
    # cleared_screen = my_clear_screen(full_screen_size, BACKGROUND_COLOUR)
    cleared_screen = jnp.broadcast_to(BACKGROUND_COLOUR, (*full_screen_size, 3))
    ...
    def fragment_shader_kinetix_circle(position, current_frag, unit_position, uniform):
        ...
        fragment = jax.lax.select(on_edge, jnp.zeros(3, dtype=jnp.uint8), colour)
    ...
    def fragment_shader_kinetix_thruster(fragment_position, current_frag, unit_position, uniform):
        ...
        if tex_frag.dtype == np.uint8:
            # LOL the alpha is just 0 or 1 masking, not making use of the full 0-255 range.
            tex_frag = (tex_frag[3] * tex_frag[:3]) + ((1 - tex_frag[3]) * current_frag)
        else:
            tex_frag = (tex_frag[3] * tex_frag[:3]) + ((1.0 - tex_frag[3]) * current_frag)

        assert tex_frag.dtype == jnp.uint8
    ...
    def render_pixels(state: EnvState):
        ...
        rectangle_edge_colours = jnp.zeros((static_params.num_polygons, 3), dtype=jnp.uint8)
