...
class LevelSampler:
    ...
    def initialize(self, leaf: TreeLeaves, pholder_level_extra=None) -> Sampler:
        """
        ...
        """
        levels = make_batch_pytree(leaf, self.capacity, fill_value=0, whichnp=jnp)
        sampler = {
            "levels": levels,
        ...
        if pholder_level_extra is not None:
            sampler["levels_extra"] = jtu.tree_map(
    ...
    def sample_replay_decision_np(self, sampler: Sampler, rng: np.random.Generator) -> bool:
        proportion_filled = float(self._proportion_filled(sampler))
        return (proportion_filled >= self.minimum_fill_ratio) and (rng.uniform() < self.replay_prob)

    def sample_replay_level(self, sampler: Sampler, rng: chex.PRNGKey) -> Tuple[Sampler, Tuple[int, TreeLeaves]]:
        """
        ...
        leaf = leaf_index(sampler["levels"], idx)
        return sampler, (idx, leaf)

    @ft.partial(jax.jit, static_argnames=("self", "num"))
    def sample_replay_levels(
        self, sampler: Sampler, rng: chex.PRNGKey, num: int
    ) -> Tuple[Sampler, Tuple[chex.Array, TreeLeaves]]:
        """
    ...
    def insert(
        self, sampler: Sampler, level: TreeLeaves, score: float, level_extra: dict = None
    ) -> Tuple[Sampler, int]:
        """
    ...
    def insert_batch(
        self, sampler: Sampler, levels: TreeLeaves, scores: chex.Array, level_extras: dict = None
    ...
    def find(self, sampler: Sampler, level: TreeLeaves) -> int:
        """
        ...
        """
        eq_tree = jtu.tree_map(lambda X, y: (X == y).reshape(self.capacity, -1).all(axis=-1), sampler["levels"], level)
        eq_tree_flat, _ = jtu.tree_flatten(eq_tree)
    ...
    def get_levels(self, sampler: Sampler, level_idx: int) -> TreeLeaves:
        """
        ...
        Returns:
            Level:
        """
        return leaf_index(sampler["levels"], level_idx)
    ...
    def get_levels_extra(self, sampler: Sampler, level_idx: int) -> dict:
        ...
        Returns:
            dict:
        """
        return jtu.tree_map(lambda x: x[level_idx], sampler["levels_extra"])
    ...
    def update(self, sampler: Sampler, idx: int, score: float, level_extra: dict = None) -> Sampler:
        ...
        if level_extra is not None:
            new_sampler["levels_extra"] = jtu.tree_map(
                lambda x, y: x.at[idx].set(y), new_sampler["levels_extra"], level_extra
            )
        return new_sampler

    def update_batch(
        self, sampler: Sampler, level_inds: chex.Array, scores: chex.Array, level_extras: dict = None
    ) -> Sampler:
        """
    ...
    def level_weights(
        self, sampler: Sampler, prioritization: Prioritization = None, prioritization_params: dict = None
    ) -> chex.Array:
        """
    ...
    def get_rank_weights_np(self, b_scores: np.ndarray, prioritization_params: dict = None):
        if prioritization_params is None:
            prioritization_params = self.prioritization_params

        # Largest score first.
        b_order = -np.argsort(b_scores)

        b_ranks = np.empty_like(b_order)
        b_ranks[b_order] = np.arange(len(b_order)) + 1

        temperature = prioritization_params["temperature"]

        b_w_s = 1 / b_ranks ** (1 / temperature)

        # normalize
        b_w_s = b_w_s / b_w_s.sum()
    ...
    def _insert_new(self, sampler: Sampler, level: TreeLeaves, score: float, level_extra: dict) -> Tuple[Sampler, int]:
        ...
        def _replace():
            new_sampler = {
                **sampler,
                "levels": jtu.tree_map(lambda x, y: x.at[idx].set(y), sampler["levels"], level),
                "scores": sampler["scores"].at[idx].set(score),
                "timestamps": sampler["timestamps"].at[idx].set(sampler["episode_count"] + 1),
    ...
    def _get_next_idx(self, sampler: Sampler) -> int:
        return jax.lax.select(sampler["size"] < self.capacity, sampler["size"], self.level_weights(sampler).argmin())
