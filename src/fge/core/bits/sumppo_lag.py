...
from fge.core.common.update_fn import update_mse, update_policy_ppo, update_policy_ppo_lag
from fge.core.envs.jax_task import JaxTask
from fge.core.utils.jax_util import myjit


@define
class SumPPOLagTrainCfg:
    gae_lambda: float
...
class SumPPOLagCfg:
    ...
    train_cfg: SumPPOLagTrainCfg

    init_lam: float  # Initial Lagrange multiplier value

    # cost_thres: float = (0.175 * 300) * 0.25  # Hard coded for toylevels. Used to update lamba (which is fixed now so this is ignored)

    def asdict(self):
        return asdict(self)


class SumPPOLag(struct.PyTreeNode):
    Cfg = SumPPOLagCfg
    TrainCfg = SumPPOLagTrainCfg
    ...
    VlC: TrainState[FloatScalar]

    lam: FloatScalar  # Lagrange multiplier, initialized to 0.0 and will be updated per iteration
    ...
    cfg: SumPPOLagCfg = struct.field(pytree_node=False)
    ...
    class PPOBatch(NamedTuple):
        ...
        Attributes:
            ...
            b_AlC (BFloat): Batch of cost advantages.
            b_QlC (BFloat): Batch of cost Q-values.
        """
    ...
    class PPOBatchQvals(NamedTuple):
        """
        A batch of Q-values used in the SumPPOLag algorithm.
        """
    ...
    @classmethod
    def create(cls, key: PRNGKey, task: JaxTask, cfg: SumPPOLagCfg) -> Self:
        """
        Creates an instance of the SumPPOLag class.
        ...
        Args:
            ...
            cfg (SumPPOLagCfg): Configuration object for the SumPPOLag algorithm.

        Returns:
            Self: An instance of the SumPPOLag class.
        """

        # Generate key data and split it into three keys for policy and value network initialization.
        key = jr.key_data(key)
        key, key_pol, key_Vl, key_VlC = jr.split(key, 4)
        ...
        # Create lambda
        lam = jnp.array(cfg.init_lam, dtype=jnp.float32)
        ...
        # Create the cost value network with optional output limits.
        VlC_def = ValueNet(V_base_cls, 1, lims=cfg.value_lims)
        VlC_tx = get_default_tx(cfg.val_lr)
        VlC = TrainState.create_from_def(key_VlC, VlC_def, (obs,), tx=VlC_tx)
        # -----------------------
        ...
        # Return an instance of the SumPPOLag class with the initialized components.
        return SumPPOLag(zero, *jr.split(key, 2), policy, Vl, VlC, lam, ent_cf, pol_lr, task, cfg)

    @property
    def train_cfg(self):
        """
        Returns the training configuration for the SumPPOLag algorithm.
        """
    ...
    def disc_gamma(self):
        """
        Returns the discount factor for future rewards used in the SumPPOLag algorithm.
        """
    ...
    def compute_bT_A_Q_C(self, data: Collector.Rollout):
        """
        Advantages and Q-values for cost
        """
        bT_VlC_nxt = jax_vmap(self.VlC.apply, rep=2)(data.T_obs_nxt).squeeze(-1)
        bT_VlC_now = jax_vmap(self.VlC.apply, rep=2)(data.T_obs_now).squeeze(-1)

        # 2: Compute Ql using GAE.
        gae_fn = ft.partial(compute_gae, self.disc_gamma, self.train_cfg.gae_lambda)

        # # Assert T_control is correct shape
        ...
        # # bT_cost = jnp.abs(data.T_control)
        # # TODO: cost is now just safety
        # bT_cost = data.T_rew
        ...
        # Compute the costs as magnitude of action
        bT_QlC, bT_AlC = jax_vmap(gae_fn)(bT_cost, bT_VlC_now, bT_VlC_nxt, bT_nextvalid)
        assert bT_QlC.shape == bT_AlC.shape == bT_cost.shape

        return bT_AlC, bT_QlC

    def make_bT_dset(self, data: Collector.Rollout) -> tuple[PPOBatch, dict]:
        bT_Al, bT_Ql = self.compute_bT_A_Q(data)
        bT_AlC, bT_QlC = self.compute_bT_A_Q_C(data)
        ...
        # Compute stats for lagrange cost advantages
        mean_C, std_C = jnp.mean(bT_AlC), jnp.std(bT_AlC)
        info_C = {
            "C_gae/min": bT_AlC.min(),
            "C_gae/q10": jnp.quantile(bT_AlC, 0.1),
            "C_gae/mean": mean_C,
            "C_gae/std": std_C,
            "C_gae/q90": jnp.quantile(bT_AlC, 0.9),
            "C_gae/max": bT_AlC.max(),
        }
        ...
        if self.train_cfg.normalize_adv:
            bT_AlC = (bT_AlC - mean_C) / (std_C + 1e-5)

        # Package the processed data into a PPOBatch object.
        bT_batch = SumPPOLag.PPOBatch(
            data.T_obs_now, data.T_control, data.T_logprob, bT_Al, bT_Ql, bT_AlC, bT_QlC
        )
    ...
    def make_bT_dset_Qvals_only(
        ...
        bT_batch = SumPPOLag.PPOBatchQvals(bT_Qvals)
        return bT_batch, info
    ...
    def update(self, data: Collector.Rollout, **kwargs) -> tuple[Self, FloatDict]:
        ...
        Returns:
            tuple:
                - Self: The updated instance of the SumPPOLag class.
                - FloatDict: A dictionary containing statistics and information about the update process.
        """

        def updates_body(alg_: SumPPOLag, b_batch: SumPPOLag.PPOBatch):
            """
            Performs a single update step for the value and policy networks.

            Args:
                alg_ (SumPPOLag): The current instance of the SumPPOLag class.
                b_batch (SumPPOLag.PPOBatch): A batch of data for training.

            Returns:
                tuple:
                    - SumPPOLag: The updated instance of the SumPPOLag class.
            ...
            alg_, valC_info = alg_.update_value_cost(b_batch)  # Update the cost value function.
            alg_, pol_info = alg_.update_policy(b_batch)  # Update the policy.
            alg_, lam_info = alg_.update_lam(b_batch)  # Update the Lagrange multiplier.
            return alg_, val_info | pol_info | valC_info | lam_info

        # Initialize a new instance of SumPPOLag for the update.
        new_self = self
    ...
    def update_value_cost(self, batch: PPOBatch) -> tuple[Self, FloatDict]:
        """
        Updates the cost value network using the provided batch of data.
        ...
        """
        # Perform the value network update using mean squared error loss.
        VlC, VlC_info = update_mse(
            self.VlC,  # Current value network.
            batch.b_QlC[
            :, None
            ],  # Q-values as targets, reshaped to match the network's output shape.
            batch.b_obs,  # Observations as inputs to the value network.
            "VC",  # Identifier for logging or debugging purposes.
            clip_grad=self.train_cfg.clip_grad_V,  # Gradient clipping value from the training configuration.
        )
        # Replace the current value network with the updated one and return the result.
        return self.replace(VlC=VlC), VlC_info

    def update_lam(self, batch: PPOBatch) -> Self:
        """
        Update the Lagrange multiplier based on the average cost from the batch.

        Args:
            batch (PPOBatch): A batch of data that includes:
                - b_AlC: Batch of cost advantages.

        Returns:
            Self: The updated instance of the SumPPO class with the updated Lagrange multiplier.
        """
        # Compute the average cost from the batch.
        avg_cost = jnp.mean(batch.b_QlC)

        # # Update the Lagrange multiplier using a simple gradient ascent step.
        # new_lam = jnp.maximum(0.0, self.lam + 0.01 * (avg_cost - self.cfg.cost_thres))

        # TODO: just keep constant, since some impossible parameters will blow up lam
        new_lam = self.lam
        ...
        # Return the updated instance with the new Lagrange multiplier.
        return self.replace(lam=new_lam), info
    ...
    def update_policy(self, batch: PPOBatch) -> tuple[Self, FloatDict]:
        ...
        policy, pol_info = update_policy_ppo_lag(
            key_entropy,
            self.policy,
            batch.b_obs,
            batch.b_control,
