...
from og.dyn_types import BControl, BObs, Obs, TBool, TControl, TFloat, TObs
...
from fge.core.common.networks import encoder_modules, ImpalaEncoder
...
from fge.core.envs.jax_task import JaxTask, TimedState, TreeLeaves
from fge.core.utils.jax_util import myjit


@define
class SumPPOTrainCfg:
    gae_lambda: float
...
class SumPPOUpdateData(NamedTuple):
...
class SumPPOCfg:
    ...
    train_cfg: SumPPOTrainCfg

    def asdict(self):
        return asdict(self)


class SumPPO(struct.PyTreeNode):
    Cfg = SumPPOCfg
    TrainCfg = SumPPOTrainCfg
    ...
    cfg: SumPPOCfg = struct.field(pytree_node=False)
    ...
    class PPOBatch(NamedTuple):
        ...
        def batch_size(self) -> int:
            ...
            return len(self.b_logprob)  # Return the length of b_logprob as the batch size.
    ...
    @classmethod
    def create(cls, key: PRNGKey, task: JaxTask, cfg: SumPPOCfg) -> Self:
        ...
        if is_obs_array:
            ...
            # encoder_module = ft.partial(ImpalaEncoder, num_blocks=1, stack_sizes=(8, 16, 16))

            base_cls = ft.partial(MergeCNN, encoder=encoder_module(), hid_sizes=cfg.pol_hids)
            V_base_cls = ft.partial(MergeCNN, encoder=encoder_module(), hid_sizes=cfg.val_hids)
        ...
        if isinstance(action_space, Discrete):
            ...
            pol_def = TanhNormal(base_cls, action_space.shape[0], scale_final=scale_final)
        ...
        # Return an instance of the SumPPO class with the initialized components.
        return SumPPO(zero, *jr.split(key, 2), policy, Vl, ent_cf, pol_lr, task, cfg)
    ...
    def get_data_from_rollout(self, rollout: Collector.Rollout) -> SumPPOUpdateData:
        return SumPPOUpdateData(
            T_obs_now=rollout.T_obs_now,
            T_obs_nxt=rollout.T_obs_nxt,
            T_control=rollout.T_control,
            T_logprob=rollout.T_logprob,
            T_rew=rollout.T_rew,
            T_trunc=rollout.T_trunc,
            T_term=rollout.T_term,
        )

    def compute_bT_A_Q(self, data: SumPPOUpdateData):
        ...
        bT_Vl_nxt = jax_vmap(self.Vl.apply, rep=2)(bT_obs_nxt).squeeze(-1)
        bT_Vl_now = jax_vmap(self.Vl.apply, rep=2)(bT_obs_now).squeeze(-1)
    ...
    def make_bT_dset(self, data: SumPPOUpdateData) -> tuple[PPOBatch, dict]:
        ...
        bT_batch = SumPPO.PPOBatch(bT_obs_now, data.T_control, data.T_logprob, bT_Al, bT_Ql)
        return bT_batch, info

    def make_bT_dset_Qvals_only(self, data: Collector.Rollout) -> tuple[PPOBatchQvals, dict]:
        ...
        bT_nextvalid = ~dones  # Shape: (b, T). This indicates whether the next state is valid.
        bT_Qvals = jax_vmap(qval_fn)(data.T_rew, bT_nextvalid)
        assert bT_Qvals.shape == data.T_rew.shape  # Ensure the shape matches the rewards tensor.
        ...
        bT_batch = SumPPO.PPOBatchQvals(bT_Qvals)
        return bT_batch, info
    ...
    def update(self, data: Collector.Rollout, **kwargs) -> tuple[Self, FloatDict]:
        def updates_body(alg_: SumPPO, b_batch: SumPPO.PPOBatch):
        ...
        # Initialize a new instance of SumPPO for the update.
        new_self = self
        ...
        for ii in range(self.train_cfg.n_update_epochs):
            ...
            assert b_dset.batch_size % n_batches == 0  # Ensure the dataset can be evenly split into batches.
            ...
            b_dset = jtu.tree_map(lambda x: x[rand_idxs], b_dset)  # Shuffle the dataset.
            mb_dset = tree_split_dims(b_dset, (n_batches, batch_size))  # Split the dataset into minibatches.

            DBG = False
            if DBG:
                # Only call updates_body once.
                new_self, info = updates_body(new_self, jtu.tree_map(lambda x: x[0], mb_dset))
            else:
    ...
    def update_value(self, batch: PPOBatch) -> tuple[Self, FloatDict]:
        ...
        Vl, Vl_info = update_mse(
            ...
            batch.b_Ql[:, None],  # Q-values as targets, reshaped to match the network's output shape.
    ...
    def update_policy(self, batch: PPOBatch) -> tuple[Self, FloatDict]:
        key_entropy = jr.split(jr.fold_in(self.key_entropy, self.policy.step), batch.batch_size)
    ...
    def set_policy_params(self, params: dict):
        ...
        if jtu.tree_structure(self.policy.params) != jtu.tree_structure(params):
            raise ValueError("Loaded params structure does not match current policy params structure.")
    ...
    def policy_det(self, obs: Obs):
        ...
        if control.ndim == 0:
            return tfd.Deterministic(control)  # Return a scalar deterministic distribution.
        else:
            return tfd.VectorDeterministic(control)  # Return a vector deterministic distribution.
    ...
    def collect_eval(self, collector: Collector) -> tuple[Collector.Rollout, dict]:
        ...
        _, rollout, info = collector.collect_batch(self.policy.apply, self.task.eval_rollout_T)
        logger.debug("jitting collect_eval... done!")
        return rollout, info

    @myjit
    def collect_eval_w_col(self, collector: Collector) -> tuple[Collector, Collector.Rollout, dict]:
        return collector.collect_batch(self.policy.apply, self.task.eval_rollout_T)

    @ft.partial(myjit, static_argnums=2)
    def collect_eval_w_col_spec_T(self, collector: Collector, T: int) -> tuple[Collector, Collector.Rollout, dict]:
    ...
    def collect_eval_det(self, collector: Collector) -> tuple[Collector.Rollout, dict]:
        ...
        _, rollout, info = collector.collect_batch(self.policy_det, self.task.eval_rollout_T)
    ...
    def collect_with_buf(
        ...
        return collector.collect_batch_with_buf(self.policy.apply, reset_buf, self.train_cfg.rollout_T)

    @ft.partial(myjit, donate_argnums=1)
    def collect_with_fn(self, collector: Collector, reset_fn: ResetFn) -> tuple[Collector, Collector.Rollout, dict]:
        return collector.collect_batch_with_fn(self.policy.apply, reset_fn, self.train_cfg.rollout_T)
