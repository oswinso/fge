...
from fge.core.envs.toylevels.viz_farr import VizFARR, VizFARR2
...
def main(
    ...
    # b_uniform = np.linspace(-1, 1, num=64)[:, None]
    # b_state: ToyLevelsJax.State = jax.vmap(env.reset_from_box)(b_uniform)
    # b_px = np.array(b_state.pos[:, 0])
    #
    # fig, ax = plt.subplots()
    # env.task_cpu.label_ic(ax)
    # ax.scatter(b_px, b_px * 0, color="C1")
    # plt.show()
