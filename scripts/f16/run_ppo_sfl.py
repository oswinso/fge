...
def main(
    ...
    sfl_cfg = SFLSampler.Cfg(
        update_buf_every=50,  # Update period T. How often to call "collect_learnable_levels".
        p_sample_base=0.1,  # 1 - rho, probability of sampling from the base distribution.
        init_sample_size=5_000,  # N. How many levels to sample before rolling out and getting top K
        sfl_buf_size=100,  # The K in Top K levels in B ranked by learnability
        T_coef=2,  # How many times to multiply with rollout_eval_T to get a non-0/1 success rate
    )
    algo_cfg = PPOSFLCfg(
        ppo=ppo_cfg,
        sfl=sfl_cfg,
